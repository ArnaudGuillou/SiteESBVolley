...
class Theme_StaticBlock_Widget extends WP_Widget {
	...
	function Theme_StaticBlock_Widget() {
		$widget_ops = array( 'classname' => 'static-block', 'description' => __('Displays the content from a Static Block. ', 'framework') );
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'static-block-widget' );
		$this->WP_Widget( 'static-block-widget', __('Static Block', 'framework'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'static_block_id' ); ?>"><?php _e('Static Block:', 'framework'); ?></label>
			<select id="<?php echo $this->get_field_id( 'static_block_id' ); ?>" name="<?php echo $this->get_field_name( 'static_block_id' ); ?>" style="width:100%;" >
			<?php 
			...
			foreach ($static_blocks as $key => $value) {
				...
				echo '<option value="'.$id.'" '. selected( $instance['static_block_id'], $id, false) .'>'.esc_attr($title).'</option>';
			} 
			?>
			</select>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_title' ); ?>"><?php _e('Show title', 'framework'); ?></label>
		</p>
